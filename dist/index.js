...
var useMemoizedFn = function (fn) {
    ...
    var memoizedFn = React.useRef(undefined);
...
var useUpdate = function () {
    ...
    return React.useCallback(function () { return setState({}); }, []);
};
...
function useInitialized(check) {
  const initializedRef = React.useRef(check);
  if (check) {
...
function useInnerVisible(outerVisible) {
  const [innerVisible, setInnerVisible] = React.useState(outerVisible);
...
const Button$1 = React.forwardRef((p, ref) => {
...
var Button = function (_a) {
    ...
    return (React.createElement(Button$1, __assign({ color: getAntdColor(type), size: getAntdSize(size), block: block, loading: loading, disabled: disabled, className: buttonClassName, onClick: onClick }, rest), children));
};
...
var Input = function (_a) {
    ...
    return (React.createElement("div", { className: wrapperClassName },
        label && (React.createElement("div", { className: "company-input-label" },
            label,
            required && React.createElement("span", { className: "company-input-required" }, "*"))),
        React.createElement("div", { className: "company-input-container" },
            React.createElement(Input$1, __assign({ type: type, placeholder: placeholder, value: value, defaultValue: defaultValue, disabled: disabled, readOnly: readOnly, maxLength: maxLength, className: inputClassName, onChange: onChange, onBlur: onBlur, onFocus: onFocus }, rest)),
            prefix && React.createElement("div", { className: "company-input-prefix" }, prefix),
            suffix && React.createElement("div", { className: "company-input-suffix" }, suffix),
            allowClear && value && !disabled && !readOnly && (React.createElement("div", { className: "company-input-clear", onClick: function () { return onChange === null || onChange === void 0 ? void 0 : onChange(''); } }, "\u2715"))),
        showCount && maxLength && (React.createElement("div", { className: "company-input-count" },
            (value || '').length,
            "/",
            maxLength)),
        errorMessage && (React.createElement("div", { className: "company-input-error-message" }, errorMessage)),
        helpText && !errorMessage && (React.createElement("div", { className: "company-input-help-text" }, helpText))));
};
...
var Card = function (_a) {
    ...
    var renderHeader = function () {
        if (!title && !subtitle && !extra)
            return null;
        return (React.createElement("div", { className: "company-card-header", style: headerStyle },
            React.createElement("div", { className: "company-card-header-content" },
                title && React.createElement("div", { className: "company-card-title" }, title),
                subtitle && React.createElement("div", { className: "company-card-subtitle" }, subtitle)),
            extra && React.createElement("div", { className: "company-card-extra" }, extra)));
    };
    var renderCover = function () {
        if (!cover)
            return null;
        return (React.createElement("div", { className: "company-card-cover" }, cover));
    };
    var renderBody = function () {
        if (!children)
            return null;
        return (React.createElement("div", { className: "company-card-body", style: bodyStyle }, children));
    };
    var renderLoading = function () {
        if (!loading)
            return null;
        return (React.createElement("div", { className: "company-card-loading-overlay" },
            React.createElement("div", { className: "company-card-loading-spinner" },
                React.createElement("div", { className: "company-card-loading-dot" }),
                React.createElement("div", { className: "company-card-loading-dot" }),
                React.createElement("div", { className: "company-card-loading-dot" }))));
    };
    return (React.createElement(Card$1, __assign({ className: cardClassName, style: style, onClick: handleClick }, rest),
        renderCover(),
        renderHeader(),
        renderBody(),
        renderLoading()));
...
var Cockpit = function (_a) {
    var _b;
    var children = _a.children, _c = _a.backTitle, backTitle = _c === void 0 ? '返回' : _c, _d = _a.backArrowContent, backArrowContent = _d === void 0 ? React.createElement(LeftOutline, { color: "#ffffff" }) : _d, rightContent = _a.rightContent, _e = _a.NavBarTitle, NavBarTitle = _e === void 0 ? '测试标题' : _e, back = _a.back, footerTabs = _a.footerTabs, initKey = _a.initKey, className = _a.className, style = _a.style, onNavigate = _a.onNavigate, onTabChange = _a.onTabChange;
    var _f = React.useState(initKey || ((_b = footerTabs === null || footerTabs === void 0 ? void 0 : footerTabs[0]) === null || _b === void 0 ? void 0 : _b.key) || 'home'), tags = _f[0], setTags = _f[1];
    var _g = React.useState(false), visible2 = _g[0], setVisible2 = _g[1];
    var _h = React.useState([]), childrenList = _h[0], setChildrenList = _h[1];
                ...
                getFooterTabs: __spreadArray(__spreadArray(__spreadArray([], footerTabs.slice(0, 2), true), [
                    {
                        key: 'showMore',
                        title: '',
                        icon: (React.createElement("div", { className: "company-cockpit-show-more-icon", onClick: function () { return setVisible2(true); } },
                            React.createElement(AppstoreOutline, null))),
                        url: '',
                    }
    ...
    var goTags = React.useCallback(function (val) {
        if (val === 'showMore')
            return;
    ...
    var tagsItem = React.useCallback(function (title, key, icon, url) {
        return (React.createElement("div", { key: key, className: "company-cockpit-tag-content", onClick: function () {
                if (url && onNavigate) {
                    onNavigate(url);
                }
            } },
            React.createElement("div", { className: "company-cockpit-tag-content-icon" }, icon),
            React.createElement("p", { className: "company-cockpit-tag-content-title" }, title)));
    }, [onNavigate]);
    var renderMenuList = React.useCallback(function () {
        return extendMenuList.map(function (item) {
            return (React.createElement("div", { className: "company-cockpit-menu-content-list", onClick: function () {
                    if (item.url && onNavigate) {
                        onNavigate(item.url);
                    }
                }, key: item.key },
                React.createElement("div", { className: "company-cockpit-menu-content-list-item" },
                    React.createElement("div", { className: "company-cockpit-menu-content-list-icon" }, item.icon),
                    React.createElement("span", null, item.title))));
        });
    }, [extendMenuList, onNavigate]);
    var innerBack = React.useCallback(function () {
        if (back) {
            back();
        }
    }, [back]);
    var cockpitClassName = "company-cockpit ".concat(className || '').trim();
    return (React.createElement("div", { className: cockpitClassName, style: style },
        React.createElement("div", { className: "company-cockpit-top-safe-area" },
            React.createElement(SafeArea, { position: "top" })),
        React.createElement("div", { className: "company-cockpit-content" },
            React.createElement("div", { className: "company-cockpit-navbar" },
                React.createElement(NavBar, { back: React.createElement("span", { className: "company-cockpit-navbar-back-title" }, backTitle), onBack: innerBack, backArrow: backArrowContent ? backArrowContent : false, right: rightContent ? rightContent : false },
                    React.createElement("span", { className: "company-cockpit-navbar-title" }, NavBarTitle))),
            childrenList && childrenList.length > 0 && (React.createElement("div", { className: "company-cockpit-list-tags" }, childrenList.map(function (item) {
                return tagsItem(item.title, item.key, item.icon, item.url);
            }))),
            React.createElement("div", { className: "company-cockpit-children-content" }, children),
            React.createElement("div", { className: "company-cockpit-footer-tabbar" },
                React.createElement(TabBar, { onChange: function (value) { return goTags(value); }, activeKey: tags }, getFooterTabs && getFooterTabs.map(function (item) { return (React.createElement(TabBar.Item, { className: "".concat(item.disable ? "company-cockpit-disabled" : ""), key: item.key, icon: item.icon, title: item.title })); })))),
        React.createElement("div", { className: "company-cockpit-bottom-safe-area" },
            React.createElement(SafeArea, { position: "bottom" })),
        React.createElement(Popup, { visible: visible2, onMaskClick: function () {
                setVisible2(false);
            }, bodyClassName: "company-cockpit-menu-popup", bodyStyle: { height: '40vh' } },
            React.createElement("div", { className: "company-cockpit-menu-popup-content" },
                React.createElement("div", { className: "company-cockpit-menu-popup-grid" }, renderMenuList())))));
};
