...
import React__default, { useState, useLayoutEffect, useMemo, useCallback } from 'react';
import { Button as Button$1, Input as Input$1, Card as Card$1, SafeArea, NavBar, TabBar, Popup } from 'antd-mobile';
...
const Button = ({ type = 'default', htmlType = 'button', size = 'middle', block = false, loading = false, disabled = false, shape = 'default', className, children, onClick, ...rest }) => {
    // 映射公司内部类型到 antd-mobile 的 color 属性
    const getAntdColor = (buttonType) => {
        switch (buttonType) {
            case 'primary':
                return 'primary';
            case 'success':
    ...
    const buttonClassName = classNames('company-button', `company-button--${type}`, `company-button--${size}`, `company-button--${shape}`, {
        'company-button--block': block,
        'company-button--loading': loading,
        'company-button--disabled': disabled,
    }, className);
    return (jsxRuntimeExports.jsx(Button$1, { color: getAntdColor(type), size: getAntdSize(size), block: block, loading: loading, disabled: disabled, className: buttonClassName, onClick: onClick, ...rest, children: children }));
};

const Input = ({ type = 'text', size = 'middle', status, showCount = false, maxLength, allowClear = false, prefix, suffix, label, errorMessage, helpText, required = false, className, placeholder, value, defaultValue, disabled = false, readOnly = false, onChange, onBlur, onFocus, ...rest }) => {
    const inputClassName = classNames('company-input', `company-input--${size}`, {
    ...
    return (jsxRuntimeExports.jsxs("div", { className: wrapperClassName, children: [label && (jsxRuntimeExports.jsxs("div", { className: "company-input-label", children: [label, required && jsxRuntimeExports.jsx("span", { className: "company-input-required", children: "*" })] })), jsxRuntimeExports.jsxs("div", { className: "company-input-container", children: [jsxRuntimeExports.jsx(Input$1, { type: type, placeholder: placeholder, value: value, defaultValue: defaultValue, disabled: disabled, readOnly: readOnly, maxLength: maxLength, className: inputClassName, onChange: onChange, onBlur: onBlur, onFocus: onFocus, ...rest }), prefix && jsxRuntimeExports.jsx("div", { className: "company-input-prefix", children: prefix }), suffix && jsxRuntimeExports.jsx("div", { className: "company-input-suffix", children: suffix }), allowClear && value && !disabled && !readOnly && (jsxRuntimeExports.jsx("div", { className: "company-input-clear", onClick: () => onChange?.(''), children: "\u2715" }))] }), showCount && maxLength && (jsxRuntimeExports.jsxs("div", { className: "company-input-count", children: [(value || '').length, "/", maxLength] })), errorMessage && (jsxRuntimeExports.jsx("div", { className: "company-input-error-message", children: errorMessage })), helpText && !errorMessage && (jsxRuntimeExports.jsx("div", { className: "company-input-help-text", children: helpText }))] }));
};

const Card = ({ title, subtitle, extra, cover, variant = 'default', size = 'medium', clickable = false, loading = false, bordered = true, children, className, style, onClick, headerStyle, bodyStyle, ...rest }) => {
    const cardClassName = classNames('company-card', `company-card--${variant}`, `company-card--${size}`, {
    ...
    const renderHeader = () => {
        if (!title && !subtitle && !extra)
            return null;
        return (jsxRuntimeExports.jsxs("div", { className: "company-card-header", style: headerStyle, children: [jsxRuntimeExports.jsxs("div", { className: "company-card-header-content", children: [title && jsxRuntimeExports.jsx("div", { className: "company-card-title", children: title }), subtitle && jsxRuntimeExports.jsx("div", { className: "company-card-subtitle", children: subtitle })] }), extra && jsxRuntimeExports.jsx("div", { className: "company-card-extra", children: extra })] }));
    };
    const renderCover = () => {
        if (!cover)
            return null;
        return (jsxRuntimeExports.jsx("div", { className: "company-card-cover", children: cover }));
    };
    const renderBody = () => {
        if (!children)
            return null;
        return (jsxRuntimeExports.jsx("div", { className: "company-card-body", style: bodyStyle, children: children }));
    };
    const renderLoading = () => {
        if (!loading)
            return null;
        return (jsxRuntimeExports.jsx("div", { className: "company-card-loading-overlay", children: jsxRuntimeExports.jsxs("div", { className: "company-card-loading-spinner", children: [jsxRuntimeExports.jsx("div", { className: "company-card-loading-dot" }), jsxRuntimeExports.jsx("div", { className: "company-card-loading-dot" }), jsxRuntimeExports.jsx("div", { className: "company-card-loading-dot" })] }) }));
    };
    return (jsxRuntimeExports.jsxs(Card$1, { className: cardClassName, style: style, onClick: handleClick, ...rest, children: [renderCover(), renderHeader(), renderBody(), renderLoading()] }));
};
...
const Cockpit = ({ children, backTitle = '返回', backArrowContent = jsxRuntimeExports.jsx(LeftOutline, { color: "#ffffff" }), rightContent, NavBarTitle = '测试标题', back, footerTabs, initKey, className, style, onNavigate, onTabChange, }) => {
    const [tags, setTags] = useState(initKey || footerTabs?.[0]?.key || 'home');
    ...
    const tagsItem = useCallback((title, key, icon, url) => {
    ...
    return (jsxRuntimeExports.jsxs("div", { className: cockpitClassName, style: style, children: [jsxRuntimeExports.jsx("div", { className: "company-cockpit-top-safe-area", children: jsxRuntimeExports.jsx(SafeArea, { position: "top" }) }), jsxRuntimeExports.jsxs("div", { className: "company-cockpit-content", children: [jsxRuntimeExports.jsx("div", { className: "company-cockpit-navbar", children: jsxRuntimeExports.jsx(NavBar, { back: jsxRuntimeExports.jsx("span", { className: "company-cockpit-navbar-back-title", children: backTitle }), onBack: innerBack, backArrow: backArrowContent ? backArrowContent : false, right: rightContent ? rightContent : false, children: jsxRuntimeExports.jsx("span", { className: "company-cockpit-navbar-title", children: NavBarTitle }) }) }), childrenList && childrenList.length > 0 && (jsxRuntimeExports.jsx("div", { className: "company-cockpit-list-tags", children: childrenList.map((item) => {
                            return tagsItem(item.title, item.key, item.icon, item.url);
                        }) })), jsxRuntimeExports.jsx("div", { className: "company-cockpit-children-content", children: children }), jsxRuntimeExports.jsx("div", { className: "company-cockpit-footer-tabbar", children: jsxRuntimeExports.jsx(TabBar, { onChange: value => goTags(value), activeKey: tags, children: getFooterTabs && getFooterTabs.map((item) => (jsxRuntimeExports.jsx(TabBar.Item, { className: `${item.disable ? "company-cockpit-disabled" : ""}`, icon: item.icon, title: item.title }, item.key))) }) })] }), jsxRuntimeExports.jsx("div", { className: "company-cockpit-bottom-safe-area", children: jsxRuntimeExports.jsx(SafeArea, { position: "bottom" }) }), jsxRuntimeExports.jsx(Popup, { visible: visible2, onMaskClick: () => {
                    setVisible2(false);
